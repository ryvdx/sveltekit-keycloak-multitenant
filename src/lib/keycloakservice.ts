...
interface KeyCloakHelperInterface {
  ...
  getLoginForwardUrl(tenantMeta: TenantMeta, csrfCode: string, urlOrigin:string, email?: string): string;
...
const KeyCloakHelper: KeyCloakHelperInterface = {
  ...
  getLoginForwardUrl : (tenantMeta: TenantMeta, csrfCode: string, urlOrigin:string, email?: string) => {
...
const KeyCloakHandle:Handle =  async ({ event, resolve }) => {
  ...
  if (event.url.pathname === LOGIN_PATH && event.request.method === 'POST' && event.url.search === '?/login') {
    ...
    try {
      ...
      loginUrl = KeyCloakHelper.getLoginForwardUrl(tenantMeta, csrfCode, event.url.origin, email);
    }
    catch (err) {
